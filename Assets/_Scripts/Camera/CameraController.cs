...
public class CameraController : MonoBehaviour
{
	...
	private void PanControll()
	{
		...
		transform.parent.transform.Translate(movement * Time.deltaTime * panSpeed, Space.Self); // move based to self space.
	}
	...
	private void RotateAroundControll()
	{
		...
		{
			transform.RotateAround(focusPoint.transform.position, Vector3.up, -90);
		}
	...
	private void ZoomControll()
	{
		...
		if (axis != 0) {
			if (Camera.main.orthographicSize > zoomMaxLimit && axis < 0 ) 
			{
				return;
			}
			if (Camera.main.orthographicSize < zoomMinLimit && axis > 0) 
			{
				return;
			}
			Camera.main.orthographicSize -= axis * zoomSpeed * Time.deltaTime;
		}
